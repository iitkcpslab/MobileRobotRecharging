...
void declarePropositions(ofstream &ofp, workspace_t workspace)
{
  ...
  {
	ofp << "(declare-const station_x_" << count1 + 1 << " Int)" << endl;
	ofp << "(declare-const station_y_" << count1 + 1 << " Int)" << endl;
  }
}
...
void definePropositions(ofstream &ofp, workspace_t workspace)
{
  ...
  for (count1 = 0; count1 < workspace.number_of_uavs; count1++)
  {
    ...
    {
	// new : mapping
	ofp << "(assert (=> (< charge_" << count1 + 1 << "_" << count2 + 1 << " " << workspace.charge_thres << ") (= r" << count1 + 1 << "chrg_" << count2 + 1 << " false)))" << endl;
	ofp << "(assert (=> (= r" << count1 + 1 << "chrg_" << count2 + 1 << " false) (< charge_" << count1 + 1 << "_" << count2 + 1 << " " << workspace.charge_thres << ")))" << endl << endl;
    }
  }
