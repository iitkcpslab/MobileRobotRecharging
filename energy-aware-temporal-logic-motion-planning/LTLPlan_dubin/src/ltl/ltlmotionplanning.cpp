...
void generateZ3File(prim_vec_t primitives, pos_vec_t obstacles, workspace_t workspace)
{
  ...
  setInitialLoopPointConstraints(ofp, workspace); //new
  ...
  writeRechargeConstraint(ofp, workspace, obstacles);  // new 
  ofp << endl;
...
void execute_ltlplan (prim_vec_t primitives, prim_cost_t prim_cost, pos_vec_t obstacles, workspace_t workspace, int choice)
{
  char command[100];
  ...
  if(choice==0) 
    ...
    trajectory_length = generateTrajectory2(primitives, prim_cost, obstacles, workspace);
...
{
   int choice;
   cout << "Please enter a choice \n 1.Trajectory generation with a specific cost.\n 2.Trajectory analysis." << endl;
   cin >> choice;
   ...
   switch(choice)
   {
     case 1 :
     {
       execute_ltlplan(primitives, prim_cost, obstacles, workspace, 0);
       printTrajectoryInfo("z3_output");
       cout << "Trajectory generation with a specific cost is done ..." << endl;
       matlab_code_generator(workspace, obstacles, getTrajectory("z3_output"), "output_matlab.m"); 
       break;
     }

     case 2 :
     {
       // set the charge to a higher value so that robot doesnt visit a charging station 
       workspace.recharge_amt = 100;
       execute_ltlplan(primitives, prim_cost, obstacles, workspace, 0);
       printTrajectoryInfo("z3_output"); // needs modi
       
       cout << "============================================================" << endl;
       // will check for loop point charge incrementally
       execute_ltlplan(primitives, prim_cost, obstacles, workspace, 1);
       printTrajectoryInfo("z3_output2");
       matlab_code_generator(workspace, obstacles, getTrajectory("z3_output2"), "output_matlab2.m"); 
       cout << "Trajectory analysis is done ..." << endl;
       break;
     }
